...
if (_side ==0) then 
	{
		[player, nil, rSIDERADIO,"TRANSPORTREQUEST_EAST"] call RE;
		_ALLTRANSPORT  = DAP_BF_EAST_TRANSPORT;
	
	};
if (_side ==1) then 
	{
		[player, nil, rSIDERADIO,"TRANSPORTREQUEST_WEST"] call RE;
		_ALLTRANSPORT  = DAP_BF_WEST_TRANSPORT;
	};
	
{
	if(!(alive _x)) then {_ALLTRANSPORT = _ALLTRANSPORT - [_x];};
}ForEach _ALLTRANSPORT;

_ACTIVETRANSPORT  = _ALLTRANSPORT;

if (count(_ACTIVETRANSPORT )>0) then 
{
	{if (((_x getVariable "DAP_BF_TRANSPORTREADY")==0) or (isPlayer(driver _x))) then {_ACTIVETRANSPORT =_ACTIVETRANSPORT -[_x];};}ForEach _ACTIVETRANSPORT;
	...
	if (((_TRANSPORT distance _unit)>DAP_BF_TRANSPORT_MINDISTANCE) and ((_TRANSPORT distance _targetpos)>DAP_BF_TRANSPORT_MINDISTANCE)) then
		{	
